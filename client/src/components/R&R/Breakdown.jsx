...
export const Breakdown = () => {
  ...
  const {totalNumberReviews, setStarSelected, starSelected} = useContext(ReviewContext);
    ...
    <span>
      {Object.keys(ratingObj).slice(0).reverse().map((key) => {
        return <div style={{width: '80%', overflow:'hidden', whiteSpace: 'nowrap', border: (starSelected[key] && starSelected.anySelected)?'1px solid red':null}} onClick = {
            () => {setStarSelected({...starSelected, [key]: !starSelected[key], 'anySelected': checkIfAnySelected({...starSelected, [key]: !starSelected[key], 'anySelected':false}) })}}>
